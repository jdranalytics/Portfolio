### VISUALIZACION CREADA PARA POWER BI
...
names(sentimentAgg) <- c("Topic", "Sentiment", "Frequency")
sentimentAgg <- sentimentAgg[order(-sentimentAgg$Frequency),]
sentimentAgg2 <- merge(x = sentimentAgg, y = termsDf, by = c("Topic"), all.x = TRUE)

plotOutput <- ggplot2::ggplot(data = sentimentAgg2, 
                              ggplot2::aes(x = Terms, y = Frequency, fill = Sentiment)) +
  ggplot2::geom_bar(stat = "identity") + 
  ggplot2::scale_fill_manual(values=c("#FF80FF", "#11EFE3", "#635BFF")) +
  ggplot2::coord_flip() +
  ggplot2::geom_text(ggplot2::aes(label=Frequency),color="white",size = 5, position = ggplot2::position_stack(vjust = 0.5)) +
  ggplot2::labs(x = "TOPIC TERMS", y = "FREQUENCY") +
