...
# Default URL
DEFAULT_URL <- "https://raw.githubusercontent.com/ringoquimico/DATA_SOURCES/main/Network_Full.csv"
...
ui <- fluidPage(
  ...
  titlePanel("NetworkApp 1.0.0"),  # Nombre de la app en titlePanel
 # h2("Análisis de Redes de Grupos de Soporte", style = "font-size: 20px;"),  # Título arriba de filtros y textbox
  sidebarLayout(
    sidebarPanel(
        ...
        tabPanel("Random Forest",
                 fluidRow(
                   column(12, h3("Resumen"), 
                         ...
                         # plotlyOutput("rf_confusion_matrix", height = "400px")
                          )
                 ),
                 fluidRow(
        ...
        tabPanel("Árbol de Decisión",
                 fluidRow(
                   column(12, style = "width: 1200px;", h3("Resumen"), 
                         ...
                         # plotOutput("dt_plot", height = "900px", width = "1200px")
                          )
                 ),
                 fluidRow(
                   column(12, h3("Predicción"), 
...
server <- function(input, output, session) {
    ...
    tryCatch({
      df <- read.csv(input$data_url, sep = ";", quote = '"', encoding = "UTF-8") %>%
        mutate(
          ...
          RESOLUTION.TIME..Days. = as.double(gsub(",", ".", RESOLUTION.TIME..Days.)),
  ...
  filtered_data <- reactive({
    ...
    df <- df %>% filter(Start.of.Month >= input$date_range[1],
  ...
  trends_data <- eventReactive(input$run_btn, {
    ...
    message("Trends data for Mean_Resolution_Groups:")
    message("Rows: ", nrow(trends))
    message("Unique values: ", paste(unique(round(trends$Mean_Resolution_Groups, 2)), collapse = ", "))
  ...
  corr_data <- eventReactive(input$run_btn, {
    ...
    message("Rows after na.omit: ", nrow(df))
    message("Head of df: ", paste(capture.output(head(df)), collapse = "\n"))
    ...
    message("Variances: ", paste(names(variances), variances, collapse = ", "))
    
    valid_cols <- names(variances)[!is.na(variances) & variances > 0]
    message("Valid columns: ", paste(valid_cols, collapse = ", "))
  ...
  output$impact_bar <- renderPlotly({
    ...
    message("Correlation matrix dimensions: ", paste(dim(corr$matrix), collapse = "x"))
    message("Row names: ", paste(rownames(corr$matrix), collapse = ", "))
    message("Column names: ", paste(colnames(corr$matrix), collapse = ", "))
    ...
    message("csat_cor dimensions: ", paste(dim(csat_cor), collapse = "x"))
    
    valid_cols <- colnames(csat_cor)[colnames(csat_cor) != "CSAT_Score"]
    message("Valid columns after filtering CSAT_Score: ", paste(valid_cols, collapse = ", "))
    ...
    message("Filtered csat_cor: ", paste(csat_cor, collapse = ", "))
    ...
    message("impact_df: ", paste(capture.output(print(impact_df)), collapse = "\n"))
...
############## RANDOM  FOREST ########################################################################
  ...
  rf_model_data <- eventReactive(input$run_btn, {
    ...
    # Verificar que RESOLVED.IN.SLA sea binario
    sla_values <- unique(rf_data$RESOLVED.IN.SLA)
    if (!all(sla_values %in% c(0, 1))) {
      message("Advertencia: RESOLVED.IN.SLA contiene valores no binarios: ", paste(sla_values, collapse = ", "))
    }
    ...
    # Calcular importancia con manejo especial
    importance <- tryCatch({
    ...
    # Calculate recommended ranges and probabilities for PROMOTER
    ...
    # Distribución de SLA para PROMOTER
    ...
    # Distribución de SLA para DETRACTOR
  ...
  # Render Confusion Matrix Plot for Random Forest
  
  output$rf_confusion_matrix <- renderPlotly({
    rf <- rf_model_data()
    if (is.null(rf)) {
      return(plot_ly() %>% layout(title = "No hay datos para la matriz de confusión"))
    }
    
    cm_table <- as.matrix(rf$cm_table)
    x_labels <- colnames(cm_table)
    y_labels <- rownames(cm_table)
    ...
    plot_ly(
      ...
      layout(
        title = "Matriz de Confusión (CV=5)",
        xaxis = list(title = "Predicción"),
        yaxis = list(title = "Real"),
        plot_bgcolor = "rgba(0,0,0,0)",
        paper_bgcolor = "rgba(0,0,0,0)",
        font = list(color = ifelse(input$toggle_mode, "white", "black"))
      )
  })
  ...
  dt_model_data <- eventReactive(input$run_btn, {
    ...
    # Verificar si RESOLVED.IN.SLA es binario
    ...
    # Configurar trainControl para calcular Accuracy
    train_control <- trainControl(method = "cv", number = 5, 
                                  classProbs = TRUE, 
                                  summaryFunction = twoClassSummary,  # Para ROC, Sensibilidad, Especificidad
                                  savePredictions = TRUE)
  ...
  output$dt_summary <- renderPrint({
    ...
    if (is.null(dt)) {
      ...
      importance_scaled <- round(100 * importance / max(importance), 2)  # Escalar a porcentaje
  ...
  output$dt_plot <- renderPlot({
    ...
    if (is.null(dt)) {
      ...
      rpart.plot(dt$model$finalModel, main = title_text, 
                 box.palette = "RdYlGn", shadow.col = "gray", nn = TRUE,
                 extra = 104,  # Muestra probabilidades y porcentaje de observaciones
                 fallen.leaves = TRUE,  # Muestra las hojas "caídas" para mejor visualización
                 type = 4,  # Muestra las etiquetas de las clases y las probabilidades
                 cex = 1)  # Tamaño del texto ajustado
    }
  }, width = 1200, height = 900)  # Aumentamos el tamaño en pantalla
  ...
  output$download_dt_plot <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (is.null(dt)) {
        png(file, width = 1200, height = 900, res = 100)  # Mayor tamaño y resolución
        plot.new()
        text(0.5, 0.5, "No hay datos suficientes para graficar el árbol", cex = 1.5)
        dev.off()
      } else {
        ...
        rpart.plot(dt$model$finalModel, 
                   ...
                   fallen.leaves = TRUE,
                   type = 4,
                   tweak = 1.2)  # Aumenta el tamaño del texto en un 20%
        dev.off()
      }
    }
  )
  ...
  dt_prediction <- eventReactive(input$predict_btn_dt, {
    ...
    # Convertir RESOLVED.IN.SLA a factor con los mismos niveles que el modelo
    new_data <- data.frame(
      RESOLUTION.GROUPS = as.numeric(input$pred_groups_dt),
      RESOLUTION.TIME..Days. = as.double(input$pred_time_dt),
      RESOLVED.IN.SLA = factor(input$pred_sla_dt, levels = c(0, 1))  # Convertir a factor con niveles 0 y 1
    )
    
    # Predicción de la clase
    pred <- predict(dt$model, new_data, type = "raw")  # Obtiene la clase predicha
    # Predicción de las probabilidades
    prob <- predict(dt$model, new_data, type = "prob")  # Obtiene las probabilidades
